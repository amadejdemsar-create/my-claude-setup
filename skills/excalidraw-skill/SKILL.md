...
## Step 0: Start the Canvas Server

The Excalidraw MCP tools connect to a **local canvas server**, not excalidraw.com. You must ensure it is running before any diagram work.
...
2. **Open the canvas in the browser**:
   ...
   ```
   This is the live Excalidraw canvas. The browser MUST be open for `export_to_image`, `get_canvas_screenshot`, and `set_viewport` to work (the frontend does the rendering).

3. **Verify the MCP connection**: Try calling `describe_scene`. If it responds, you're connected. If not, the canvas server is not running or not reachable at `http://localhost:3111`.

**IMPORTANT:** Do NOT open `excalidraw.com` for MCP-driven diagram work. The MCP tools only talk to the local canvas server at `http://localhost:3111`. The canvas stores elements in memory only; restarting it clears everything. Use `export_scene` or `snapshot_scene` for persistence.

### Architecture

```
Browser (http://localhost:3111)  <-- you see the live canvas here
     ↕ WebSocket sync
Canvas Server (Express, port 3000)
     ↕ HTTP (REST API)
MCP Server (stdio, auto-launched by Claude Code)
     ↕ stdio (MCP protocol)
Claude Code
```
...
**REST API mode** (fallback): If MCP tools are not available, use HTTP endpoints at `http://localhost:3111`. See the cheatsheet for REST payloads. Note the format differences in the table below; REST and MCP accept slightly different field names.
...
2. Optional: `curl -X DELETE http://localhost:3111/api/elements/clear`
...
```bash
curl -X POST http://localhost:3111/api/elements/batch \
...
**REST mode:**
```bash
curl -X POST http://localhost:3111/api/elements/from-mermaid \
  -H "Content-Type: application/json" \
  -d '{"mermaid": "graph TD\n  A --> B\n  B --> C"}'
```
